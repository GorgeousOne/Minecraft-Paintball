...
public class SchemUtil {
	...
	public static void pasteSchem(File file, Location origin) {
		Clipboard clipboard = loadClipboard(file);
		pasteClipboard(clipboard, origin, true);
	}
	
	public static void pasteSchemWithBackup(File file, Location origin, String backupName, JavaPlugin plugin) {
		Clipboard clipboard = loadClipboard(file);
	...
	public static void resetSchemFromBackup(String backupName, Location origin, JavaPlugin plugin) {
		File backupFile = new File(plugin.getDataFolder() + BACKUPS_FOLDER + backupName + ".schem");
